...
Задание 19.4.8 {
     ...
     * Добавьте наушники и зеркало в таблицу. Сделайте COMMIT. После этого добавьте рюкзак и выполните ROLLBACK.        */
        ...
        delete from products where id in (19);      } // удалил "набор посуды" который неясно что тут забыл
            ...
            /**         Представляю два варанта кода sql, какой лучше по мнению в мире?                                 */
        SELECT cl."name", o.address from clients cl
        JOIN orders o ON cl.id = o.client_id
        WHERE cl.id IN (3,5)
        ORDER BY o.address ;

        SELECT DISTINCT cl."name", o.address FROM clients cl, orders o
        WHERE cl.id in (3,5) AND cl.id = o.client_id
        ORDER BY o.address ;
        /** первый вариант является более современным и рекомендуется для использования.
         * Он использует явное объединение таблиц с помощью оператора JOIN, что делает код более читаемым и понятным.   */
