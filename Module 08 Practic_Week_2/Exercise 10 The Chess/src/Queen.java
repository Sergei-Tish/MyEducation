public class Queen extends ChessPiece {
    ...
    public boolean getTrueAndPrint() {
        System.out.print(" -=TRUE=- ");//////////////
        return true;
    }
    public boolean getFalseAndPrint() {
    ...
    public boolean canMoveToPosition(ChessBoard chessBoard, int line, int column, int toLine, int toColumn) {
        ...
        System.out.print("int line " + line + ", int column " +column+ ", int toLine "+toLine+ ", int toColumn " + toColumn);
        boolean myPath = false;
        if (chessBoard.board[toLine][toColumn] != null &&
                chessBoard.board[toLine][toColumn].getColor().equals(this.getColor())) {
            System.out.println("kuda dvigaus - zanyato"); //////////////
            return false;
        }
        //for horizontal move
        if (toLine == line && toColumn > column) {
            for (int i = 1; i < 8; i++) {
                if (toColumn == (column + i)) {
                    return getTrueAndPrint();
                } else if (chessBoard.board[toLine][column + i] != null) {
                    return getFalseAndPrint();
                }
            }
        }
        if (toLine == line && toColumn < column) {
        ...
        System.out.print(" ETO ne podoshlo ");
        return getFalseAndPrint();
    }
